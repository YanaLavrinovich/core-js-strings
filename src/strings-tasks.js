...
function isPalindrome(str) {
  let testedString = str.toLowerCase();
  testedString = testedString.replace(/\W/g, '');
  return testedString === reverseString(testedString);
}
...
function findLongestWord(sentence) {
  ...
  let longestWord = '';
  for (let index = 0; index < massive.length; index += 1) {
    const word = massive[index];
...
function reverseWords(str) {
  ...
  for (let massiveIndex = 0; massiveIndex < massive.length; massiveIndex += 1) {
    const word = massive[massiveIndex];
    let reverseWord = '';
    for (let index = word.length - 1; index >= 0; index -= 1) {
      reverseWord = reverseWord.concat(word.at(index));
    }
    massive[massiveIndex] = reverseWord;
  }
  return massive.join(' ');
}
...
function invertCase(str) {
  let result = '';
  for (let index = 0; index < str.length; index += 1) {
    const charIndex = str.charCodeAt(index);
    if (charIndex >= 65 && charIndex <= 90) {
      result = result.concat(str.charAt(index).toLowerCase());
    } else if (charIndex >= 97 && charIndex <= 122) {
      result = result.concat(str.charAt(index).toUpperCase());
    } else {
...
function encodeToRot13(str) {
  let result = '';
  for (let index = 0; index < str.length; index += 1) {
    let char = str.at(index);
    const charCode = char.charCodeAt(0);
    if (char.match(/\w/g)) {
      char = String.fromCharCode(
        charCode + (char.toLowerCase() <= 'm' ? 13 : -13)
      );
    }
...
function getCardId(value) {
  ...
  switch (cardValue) {
    ...
    case 'K':
      ...
      break;
    default:
      result = Number.parseInt(cardValue, 10);
  }
